...
class HoerspielFolgeErzeugenTask(WriteTask):
    ...
    def __init__(self, tg, hoerspiel_client, display_url_origin: str = "",
        ...
        super().__init__(
            ...
            description=(
                ...
                "(HFE-3, E-HFE-6). »paula« wenn die Mutter sagt 'für Paula'; "
                "»neko« wenn sie 'für Neko' sagt. Default: paula.\n\n"
    ...
    def propose(self, arguments, turn_context):
        ...
        # Fallback "paula" für Backward-Compat (fehlender Parameter).
        kind_id = args.get("kind_id", "paula")  # Fallback paula für Backward-Compat
        if kind_id not in self._client_by_kind_id:
            # LLM hat einen unbekannten Wert geliefert — Hartfall auf paula
            logger.warning("Hörspiel-Task: unbekannte kind_id %r, falle auf paula", kind_id)
            kind_id = "paula"
