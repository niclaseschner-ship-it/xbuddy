...
# HFE-3 Sub-Case 1: Alter-Konstante für Mia (V1 hart, Mehr-Kind V2).
MIA_ALTER = 4
...
def propose(*, hoerspiel_client, is_member_fn, from_user_id,
    ...
    if len(idee_bereinigt) < _IDEE_MIN_ZEICHEN:
        logger.info(
            ...
            "Idee — rufe GET /themen?alter=%d (HFE-3)", MIA_ALTER)
...
def _baue_themen_rueckfrage(hoerspiel_client) -> str:
    ...
    themen: list[str] = []
    try:
        themen = hoerspiel_client.themen_lesen(MIA_ALTER)
    except HoerspielClientError as exc:
        logger.warning(
