...
def _tablet(id="tablet-paula-01", name="Tablet Paula"):
...
def test_finde_geraet_exakter_name_case_insensitive():
    client = FakeGeraeteClient([_tablet(name="Tablet Paula")])
    g = finde_geraet(client, "tablet paula")
    assert g is not None
    assert g["id"] == "tablet-paula-01"


def test_finde_geraet_substring():
    client = FakeGeraeteClient([_tablet(name="Tablet Paula")])
    assert finde_geraet(client, "paula")["id"] == "tablet-paula-01"
    assert finde_geraet(client, "PAULA")["id"] == "tablet-paula-01"


def test_finde_geraet_kein_treffer_ist_none():
    client = FakeGeraeteClient([_tablet(name="Tablet Paula")])
    assert finde_geraet(client, "Neko") is None
    assert finde_geraet(client, "") is None


def test_baue_pairing_link_zeigt_auf_funnel_und_pair_endpoint():
    link = baue_pairing_link("tablet-paula-01", BOT_TOKEN, ORIGIN)
...
def test_erwachsener_a_erzeugt_link_und_schickt_dm():
    ...
    reply = task.execute({"geraet_name": "Paula"}, _turn(from_user_id=ERWACHSENER_A))
    assert "Paula" in reply
...
def test_erwachsener_b_erzeugt_link_und_schickt_dm():
    ...
    tg = FakeTelegram()
    task = _task(client, tg, erwachsene_ids={ERWACHSENER_A, ERWACHSENER_B})
    reply = task.execute({"geraet_name": "Paula"}, _turn(from_user_id=ERWACHSENER_B))
    assert "Paula" in reply
    assert len(tg.sent) == 1
...
def test_re_pair_fuer_bereits_gepairtes_geraet_erlaubt():
    ...
    geraet = _tablet(id="tablet-paula-01", name="Tablet Paula")
    ...
    assert session_cookie.verify_pairing(token, BOT_TOKEN) == "tablet-paula-01"
...
def test_kein_erwachsener_abgelehnt_kein_token_kein_send():
    ...
    task = _task(client, tg, erwachsene_ids={ERWACHSENER_A})
    reply = task.execute(
        {"geraet_name": "Paula"}, _turn(from_user_id=KEIN_ERWACHSENER))
...
def test_geraet_nicht_gefunden():
    ...
    reply = task.execute({"geraet_name": "Neko"}, _turn(from_user_id=ERWACHSENER_A))
    assert reply == GERAET_NICHT_GEFUNDEN_FMT % "Neko"
    assert tg.sent == []
...
def test_erwachsene_quelle_aus_familie_client():
    ...
    reply_a = task.execute({"geraet_name": "Paula"}, _turn(from_user_id=ERWACHSENER_A))
    assert "Paula" in reply_a

    # ERWACHSENER_B: nicht in der Menge → abgewiesen.
    tg.sent.clear()
    client_a.liste_calls = 0
    reply_b = task.execute({"geraet_name": "Paula"}, _turn(from_user_id=ERWACHSENER_B))
