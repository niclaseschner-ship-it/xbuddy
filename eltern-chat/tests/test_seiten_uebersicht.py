...
class TestMehrdeutigkeitAlsStringToolResult:
    ...
    def test_signal_mehrdeutig_enthaelt_beide_labels(self):
        ...
        assert "Panel mias-panel-01" in result
        assert "Panel mias-panel-01 bearbeiten" in result
    ...
    def test_task_mehrdeutig_gibt_string_als_tool_result(self):
        ...
        result = task.run({"suchbegriff": "mias-panel-01"}, ctx)
        assert isinstance(result, str)
        assert "Panel mias-panel-01" in result
        assert "aktion=match" in result
    ...
    def test_roundtrip_disambiguation_ansicht(self):
        ...
        Simuliert den Mia-Panel-Live-Bug (T549-Fix2):
          Runde inventar (q='mias-panel-01') → inventar_text mit label + key.
        ...
        assert "Panel mias-panel-01" in result1
        assert "panel-mias-panel-01" in result1
        assert "Panel mias-panel-01 bearbeiten" in result1
        ...
        assert isinstance(result2, str)
        assert "https://hub.local/controller/app-panel/mias-panel-01" in result2, (
    ...
    def test_roundtrip_disambiguation_editor(self):
        ...
        Runde match (q='Panel mias-panel-01 bearbeiten') → URL zum Editor.
        """
        client = FakeSeitenClient(self._inventar_mia())
        result = seiten_uebersicht(
            chat_id=100, from_user_id=42,
...
class TestAktionMatchEquality:
    ...
    def test_aktion_match_label_equality_bei_praefix_geschwistern(self):
        ...
        assert isinstance(result, str)
        assert "https://hub.local/controller/app-panel/mias-panel-01" in result, (
