...
DEFAULTS = {
    ...
    # DEPRECATED (#1262): Der Foto-Pfad läuft jetzt über den Foto-Adapter
    ...
    # `multimodal_provider` wird nicht mehr ausgewertet — Entfernen ist PR2.
    "multimodal_provider": "",      # DEPRECATED #1262 (Vendor via Foto-Slot)
...
class Config:
    ...
    def __init__(self, bot_token, provider_api_key, provider, provider_model,
        ...
        # DEPRECATED (#1262): der Foto-Pfad holt den Key jetzt aus dem
        # Zugangsdaten-Store über den Foto-Slot (ZD-5), nicht mehr aus
        # `multimodal_api_key`; `multimodal_provider` ist über den Slot-Vendor
        # gepinnt. Beide bleiben in V1 als Feld erhalten (Entfernen = PR2).
        # `multimodal_model` wird weiter ausgewertet (Modell-Override, #1262).
        self.multimodal_provider = multimodal_provider  # DEPRECATED #1262 (Slot-Vendor)
        self.multimodal_api_key = multimodal_api_key    # DEPRECATED #1262 (Foto-Slot/ZD-5)
...
def resolve(config_path, zd=None):
    ...
    # provider_api_key beim Aufbau des Adapters (Aufrufer-Petrantwortung).
