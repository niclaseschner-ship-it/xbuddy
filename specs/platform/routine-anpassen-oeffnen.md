...
> Status: V1 · Refs #678 (MVP-Sammler, Funktion 2 „Routine-Anpassen"), RAT-16
...
## RAO-6 — Mini-App-URL und `web_app`-Inline-Button
...
die Mini-App-URL. V1 nutzt die `127.0.0.1`-Same-Host-Naht (MAD-7); eine
spätere `Authorization: tma`-Härtung folgt der gemeinsamen
Mini-App-Auth-Folge-Ticket-Strecke (MAD-7 V1.x).
...
- `specs/platform/routine-zeiten-setzen.md` — RZS (Single-Value-Schnellsatz,
  co-existiert)
- `specs/platform/routine-punkte-setzen.md` — RPS (deprecated, durch RAO+
  Mini-App ersetzt)
