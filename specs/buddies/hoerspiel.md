...
> Status: V1 · Refs #729, #907 (Mehr-Instanz-Cut Mia + Finn, RAT-17)
...
V1 läuft mit **zwei expliziten Instanzen** Mia + Finn, handverdrahtet
...
### HSP-28a — Mehr-Instanz-Realität V1: Mia + Finn handverdrahtet (RAT-17)
V1 läuft mit **zwei expliziten Hörspiel-Instanzen** Mia und Finn. Diese
...
| Finn    | `xbuddy-hoerspiel-finn.service`    | 5054 oder nächster freier Port aus PORT-2-Block | `xbuddy-data/hoerspiel/finn/`       | `/display/hoerspiel/finn/<view>` · `/api/v1/hoerspiel/finn/<resource>`   |
...
**Test-Klausel:** JS-Unit-Test (JSDOM, z. B. `hoerspiel/tests/test_eltern_js.py` oder
`hoerspiel/tests/eltern.test.js`) prüft: (1) `window.location.hash = "#folgen"` zur
Ladezeit setzt aktiven Tab auf Folgen (Attribut `data-tab-active="folgen"` am Tab-Element);
(2) `hashchange`-Event mit anderem Hash-Wert schaltet den aktiven Tab live um, ohne Reload
und ohne erneute Auth-Validierung. Bind-Punkt für Implementierungs-Track.
...
**UI-Kollaps für Mia+Finn (Variante D).** Der Audio-Ausgabe-Schalter
...
- Beim Laden des Einstellungen-Reiters fetcht die Mini-App **beide**
  Service-Configs parallel (`GET /config` an Mia- und Finn-Instanz).
  ...
  „Audio-Ausgabe weicht zwischen Mia und Finn ab — Speichern setzt beide
  gleich."
- Beim Save löst der Speichern-Knopf **zwei parallele `PATCH /config`**
  aus (Mia- und Finn-Instanz), beide mit demselben `audio_ziel`-Wert.
...
HSP-`config.json` (HSP-27 ergänzt um dieses Feld; Mia-Instanz und
Finn-Instanz konfigurieren es unabhängig im Backend). Es lebt **nicht**
in `familie.json` (kein FAM-14) und **nicht** als Geräte-Capability in
`geraete.json` (kein `audio_aktiv`-Feld). Globalität für Mia+Finn wird
