"""tools.medien_store — defensive Normalisierung beim Ingest.

Domain-neutrale Library: HEIC->JPEG, Thumbnail, Poster-Frame, Aufnahmedatum,
Video-Transcode. Kein photo/-Bezug, kein Flask.

Public-API: normalisiere, NormResult, NormalizeError, THUMBNAIL_MAX_KANTE.
"""
...
class NormalizeError(Exception):
    """Das Medium konnte nicht normalisiert werden (unbekanntes/kaputtes Format)."""
...
class NormResult:
    ...
    `aufgenommen` ist der ISO-8601-Aufnahmestempel oder None.
    `dauer` ist die Video-Dauer in Sekunden (float) oder None.
    """
...
def normalisiere(rohbytes, dateiname):
    """Normalisiert ein eingehendes Medium.

    Erkennt Foto vs. Video an der Datei-Endung und delegiert.
    Hebt NormalizeError bei leeren Daten oder unbekanntem/kaputtem Format.
    """
    if not rohbytes:
...
def _normalisiere_foto(rohbytes):
    """Foto -> JPEG + Thumbnail; Aufnahmedatum aus EXIF.

    HEIC wird ueber den registrierten pillow-heif-Opener gelesen.
    EXIF-Aufnahmedatum wird VOR dem Re-Encode extrahiert.
    EXIF-Orientierung wird angewandt.
    """
    try:
    ...
    return NormResult(
        typ=store_mod.TYP_FOTO, daten=voll, endung=".jpg",
...
def _exif_aufnahmedatum(bild):
    """Liest das EXIF-Aufnahmedatum als ISO-8601 — oder None."""
    try:
        exif = bild.getexif()
    except Exception:
        return None
...
def _exif_zeit_zu_iso(roh):
    """`YYYY:MM:DD HH:MM:SS` -> `YYYY-MM-DDTHH:MM:SS`. Ungueltig -> None."""
    roh = roh.strip()
    try:
...
def _normalisiere_video(rohbytes, endung):
    ...
    Bereits web-taugliche MP4/H.264-Eingaben werden durchgereicht (E-PHOTO-9).
    Andere Container/Codecs werden per ffmpeg nach MP4/H.264 transcodiert.
    """
    with tempfile.TemporaryDirectory(prefix="medien-norm-") as tmp:
    ...
    return NormResult(
        typ=store_mod.TYP_VIDEO, daten=voll, endung=".mp4",
        thumbnail=thumb, thumbnail_endung=".jpg",
...
def _ffprobe(pfad):
    ...
    try:
        ...
        logger.warning("ffprobe fehlgeschlagen fuer %s: %s", pfad, e)
        return {}
    if out.returncode != 0:
        return {}
    try:
...
def _container_aufnahmedatum(probe):
    """`creation_time` aus Format-/Stream-Tags als ISO-8601 — oder None."""
...
def _transcode_mp4(eingang, ausgang):
    """HEVC/MOV -> MP4/H.264 via ffmpeg. Hebt NormalizeError bei Fehler."""
    try:
        out = subprocess.run(
            ["ffmpeg", "-y", "-i", eingang,
...
def _poster_frame(video_pfad, tmp):
    """Poster-Frame als JPEG-Bytes: erster Frame via ffmpeg, auf Thumbnail-Groesse."""
