...
Instanz-Liste (mia, finn) als window.__HSP_INSTANZEN__ (kind_id+name,
foto_url best-effort/null); emil nicht enthalten; Liste an EINER
...
def test_instanz_liste_traegt_mia_und_finn(client):
    """AC-A2: GET /seiten/hoerspiel/player — gerendertes HTML enthaelt
    window.__HSP_INSTANZEN__ mit genau mia+finn (kind_id), kein emil."""
    ...
    assert "mia" in kind_ids, f"mia fehlt in Instanz-Liste: {kind_ids}"
    assert "finn"  in kind_ids, f"finn fehlt in Instanz-Liste: {kind_ids}"
    assert "emil" not in kind_ids, \
        f"emil darf nicht in Instanz-Liste sein (#1263 deferred): {kind_ids}"
...
def test_instanz_liste_genau_zwei_eintraege(client):
    """AC-A2: V1 provisioniert exakt zwei Instanzen (mia + finn, PORT-2)."""
    resp = client.get("/seiten/hoerspiel/player")
    assert resp.status_code == 200
    body = resp.get_data(as_text=True)
    start = body.index("window.__HSP_INSTANZEN__ =") + len("window.__HSP_INSTANZEN__ =")
    ...
    assert len(instanzen) == 2, \
        f"Erwartet 2 Instanzen (mia+finn), erhalten {len(instanzen)}: {instanzen}"
