...
def test_shell_pwa_sw_html_timeout():
    ...
    - Static-Assets/cacheFirst unpetraendert (kein Timeout dort).
    """
    src = _read_sw_js()
    nf_start = src.find("function networkFirst")
    assert nf_start != -1, "networkFirst-Funktion nicht gefunden"
    ...
    # cacheFirst darf KEIN Timeout tragen (Static unpetraendert)
    ...
    assert "Promise.race" not in cf_body, (
        "SHELL-PWA-SW/AC3: cacheFirst (Static-Assets) darf KEIN Timeout tragen — unpetraendert"
    )
