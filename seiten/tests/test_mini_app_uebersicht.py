...
def test_shell10_mau_panel_eintrag_hat_shell_urls(monkeypatch):
    ...
    seiten_main.configure(
        ...
        tailscale_origin="https://buddyboard.demo-tailnet.ts.net",
    )
    monkeypatch.setattr(
        seiten_main, "hole_panels",
        lambda: [{"panel_id": "mias-panel-01", "display_id": "tablet-01"}]
    )
    ...
    assert panel["shell_urls"]["heim"] == "http://192.168.1.1:8443/shell/mias-panel-01", \
        "shell_urls.heim hat falschen Wert — SHELL-10 MAU"
    assert panel["shell_urls"]["tailscale"] == \
        "https://buddyboard.demo-tailnet.ts.net/shell/mias-panel-01", \
