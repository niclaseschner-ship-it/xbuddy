"""Tests fuer T1324 / PWAM-3 — sw_scope-Naht essen-einkauf.

Prüft, dass das ausgelieferte HTML den SW-Scope aus pwa_mantel.REGISTRY['einkauf']
...
Lauf:
  python3 -m pytest seiten/tests/test_essen_einkauf.py -q
"""
...
def test_ac1_sw_scope_in_html_aus_registry(client):
    """T1324 / PWAM-3: gerendertes HTML enthaelt den SW-Scope aus REGISTRY['einkauf'].

    Kein hartkodiertes Literal im Template — {{ sw_scope }} wird mit dem
    pwa_mantel.REGISTRY['einkauf'].sw_scope-Wert substituiert.
    """
    ...
    erwartet_scope = pwa_mantel.REGISTRY["einkauf"].sw_scope
