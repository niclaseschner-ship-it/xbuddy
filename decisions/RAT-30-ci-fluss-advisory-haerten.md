...
- **Flip-Kommando (Referenz, wenn die Serie steht):** die beiden Check-Namen zu `required_status_checks` des Rulesets hinzufügen, z. B.
  ```
  gh api -X PUT repos/emilsonntag-ship-it/xbuddy/rulesets/17352637 \
    --input <ruleset.json>   # required_status_checks.required_status_checks[] += {context: "ruff"}, {context: "lint-imports"}
  ```
  (Ruleset zuerst per `gh api repos/emilsonntag-ship-it/xbuddy/rulesets/17352637` ziehen, `required_status_checks`-Regel um die zwei Kontexte ergänzen, zurück-PUTten — closes-guard bleibt erhalten.)
