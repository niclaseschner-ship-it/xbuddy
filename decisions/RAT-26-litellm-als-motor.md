...
5. **Telemetrie bleibt Hand:** `telemetry.write_call` → `provider_calls.jsonl` (SVC-5) bleibt SSoT, `pricing.py` bleibt Preis-SSoT — **nicht** litellm-native-Callbacks / `model_cost` (Zahl-Stabilität bei Rollback + Preis-Drift-Schutz).
