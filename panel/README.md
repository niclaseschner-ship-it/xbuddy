...
`panel_id`, abgeleitete `source_id` (`app-panel:<panel_id>`) und die getrennten
Felder `config` (Tuning, PANEL-8) und `tiles` (Daten, PANEL-3). Eine Instanz
beschreibt genau eine Familie (PREG-1). Die Registry besitzt diese Daten und
stellt sie über eine HTTP-Schnittstelle bereit (PREG-13/14/15); Konsumenten
(Panel-Seite) sind Nutzer, kein eigenes Panel-Modell.
...
Schreib-Schnittstelle (PREG-15), die `panel_id`-Vergabe (PREG-6). Out-of-Scope
(Welle 2): Kopieren/Löschen/Tile-Editing (OPEN-PREG-A), Reconcile-Pfad
(OPEN-PREG-B), Eltern-Chat-Skill (OPEN-PREG-C), Tile-Sets/`geraet_id`
(OPEN-PREG-D).
...
| GET     | `/api/v1/panels/<panel_id>/config.json` | PREG-14   | `config`-Feld als eigenständiges Dokument |
| GET     | `/api/v1/panels/<panel_id>/tiles.json`  | PREG-14   | `tiles`-Feld als eigenständiges Dokument |
| POST    | `/api/v1/panels/`                     | PREG-15     | Panel anlegen, atomar; Display-Validierung (PREG-7) |

`POST` erwartet `{slug, config?, tiles?}` und liefert die angelegte Instanz
inkl. vom Server vergebener `panel_id` (PREG-6) und abgeleitetem `source_id`.
Fehlerfälle: fehlendes Pflichtfeld oder verschachteltes `query` in `tiles`
(PANEL-7) → **400**; Disk-Schreibfehler → **503** (panels.json bleibt unverändert).
...
Familienspezifische Werte (die Panels) leben in `panels.json`. Pfad bleibt Env/CLI:
