...
delipetrable_kind/werft_mockup_path am F5-Stempel).
...
# ---------- PW-83: delipetrable_kind-Logik in check_werft_verdict ----------
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    ok, r = run_werft("  werft: true\n")
    check("PW-83 delipetrable_kind fehlt -> deny", ok is False and "delipetrable_kind fehlt" in r)

    # Kill-Kriterium (Codex): non_ui ohne Evidenz -> deny.
    ok, r = run_werft("  werft: true\n  delipetrable_kind: non_ui\n")
    check("PW-83 non_ui ohne delipetrable_evidence -> deny", ok is False and "delipetrable_evidence" in r)

    ok, _ = run_werft("  werft: true\n  delipetrable_kind: non_ui\n  delipetrable_evidence: hoerspiel.py:88 kein UI\n")
    check("PW-83 non_ui + Evidenz -> allow", ok is True)


# ---------- Kill-Kriterium: delipetrable_kind geht in den Hash ----------
h_ui = srg.compute_verdict_hash("verdict: ready\narchitecture_class: nachzeichnen\naxes:\n  werft: true\n  delipetrable_kind: ui_build\n")
h_non = srg.compute_verdict_hash("verdict: ready\narchitecture_class: nachzeichnen\naxes:\n  werft: true\n  delipetrable_kind: non_ui\n")
check("PW-83 Hash aendert sich bei delipetrable_kind-Flip", h_ui is not None and h_ui != h_non)
