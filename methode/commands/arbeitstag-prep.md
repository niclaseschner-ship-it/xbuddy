...
1. **Kandidaten sammeln:**
   `gh issue list --repo emilsonntag-ship-it/xbuddy --label "status:spec" --state open --json number,title,labels`
...
2. **Pro Kandidat zuerst CLAIMEN, dann `xbuddy-watchdog-prep` dispatchen** (PW-33, 2026-06-09):
   - **Claim setzen** (Lock-Semantik — niemand anders prept dieses Ticket parallel):
     ```bash
     gh issue edit <nr> --repo emilsonntag-ship-it/xbuddy --remove-label status:spec --add-label status:spec-in-progress  # status_rollback_guard:skip prep-claim
     ```
   ...
   - **Dann Dispatch** (Agent-Tool, `subagent_type: "xbuddy-watchdog-prep"`).
     ...
     `parent_ticket: emilsonntag-ship-it/xbuddy#<nr>` und `mode: read` tragen
   ...
   **SURFACE statt blind stempeln (Batch-Schutz):** Taucht beim Ausführen der
   ...
   - `stempeln` → **ZWEI Schritte in der Reihenfolge** (PW-26-RATIFIZIERT
     ...
     3. **Label-Flip — prep-Release auf ready** (Skip-Marker, PW-33):
        `gh issue edit <nr> --repo emilsonntag-ship-it/xbuddy --remove-label status:spec-in-progress --add-label status:ready  # status_rollback_guard:skip prep-release-forward (Nic-Stempel)`.
   ...
   - **Schließen-Karte** — Nic wählt `schließen` / `parken`:
     `gh issue close <nr> --repo emilsonntag-ship-it/xbuddy --reason "<dup|completed|not_planned>"`.
...
ein Ticket überhaupt aufnimmst, kurz prüfen, ob sein Delipetrable nicht schon auf
