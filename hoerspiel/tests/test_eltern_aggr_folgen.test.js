...
/**
 * Test 3 — Befund-4 (#1263): Multi-Kind-PATCH tolerant bei unserviertem niclas.
 *
 ...
 */
test("Befund-4 (#1263): _patchBeideConfigs tolerant bei niclas-404 — kein harter Fehler", async () => {
  // Routed fetch: paula+neko config PATCH → 200; niclas → 404 (nicht provisioniert).
  const routedFetch = makeRoutedFetchSpy([
    { match: /\/hoerspiel\/paula\/config/, status: 200, json: { playback_tempo: 1.0 } },
    { match: /\/hoerspiel\/neko\/config/,  status: 200, json: { playback_tempo: 1.0 } },
