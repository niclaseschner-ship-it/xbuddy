...
perspektive/serien_name) in den User-Kontext; eine niclas-Folge nennt nie
Paula/Neko, eine paula-Folge trägt Paula + den Serien-Namen (transitionaler
...
def test_paula_folge_traegt_namen_und_serien_fallback():
    """AC2: paula-Folge trägt „Paula" + Serien-Name (Fallback, serien_name leer)."""
    llm = _RecordingLLM()
    llm_service.erzeuge_folgen_vorschlag(
        idee="Stigi findet eine Feder", bible="", historie="",
        naechste_nummer=5, llm=llm,
        name="Paula", alter=4, serien_name="")  # serien_name leer → Fallback
    user = llm.last_user
    assert "Paula" in user
...
def test_niclas_folge_nennt_nie_paula_oder_neko():
    ...
    # Transitionaler Paula/Stigi-Fallback darf bei gesetzter Serie NICHT greifen.
...
def test_alt_aufruf_ohne_instanz_bleibt_gerahmt():
    ...
    keine leere/ungerahmte Story (paula/neko byte-gleich zur alten Rahmung)."""
    llm = _RecordingLLM()
    llm_service.erzeuge_folgen_vorschlag(
